    ...
    def on_any_event(self, event):
        ...
        if (not event.is_directory) and (event.event_type == 'closed'):
            try:
                ...
                for pos in prev_pos:
                    self.canva.create_oval((pos[0] / scale) - 7, (pos[1] / scale) - 7,
                                           ((pos[0] / scale) + 7),
                                           ((pos[1] / scale) + 7), fill="black", outline="")

                # normalize prev_pos with currently detected list.
                current_poses = []
                ...
                prev_pos.sort(reverse=True)  # keep always ordered list to simulate tracking same person.
            ...
            et1 = time.time()
            tt1 = et1 - st1
            print("[INFO] File change trigger over. Time taken: {:.4f} s _______________________".format(tt1))
